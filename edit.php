	...
	<body class="littlesamakfox">
        <header>
            <nav class="navbar navbar-expand-md navbar-dark fixed-top bg-littlesamakfox" style="padding:0.5rem 1rem">
                <img src="/assets/img/hr_logo.png" style="height:40px" class="mr-2"><a class="navbar-brand" href="/main.html" >Layton</a>
                ...
                <div class="collapse navbar-collapse" id="navbarCollapse">
                        <ul class="navbar-nav mr-auto">
                            <li class="nav-item">
                                <a class="nav-link" href="/main.html">메인화면<span class="sr-only">(current)</span></a>
                            </li>
                            ...
                            <li class="nav-item active">
                                <a class="nav-link" href="/thread.php">계시판</a>
                            </li>
							<li class="nav-item">
						...
						<ul class="navbar-nav">
								...
								<li class="nav-item">
									<a class="nav-link" href="#"><i class="icon ion-social-facebook unvisible-md"></i></a>
								</li>
						</ul>
                </div>
            </nav>
		...
		<div class="register">
			<div class="intro">
                ...
                <h1 class="mb-4 mt-4 text-center">글수정 </h3>
                <p class="text-center">
			...
			<?PHP
			  ...
			  #------------- 해당 글의 정보를 가져온다 -------------------#
			  $query = "select name, subject, article, userfile, userfile2
				from thread where uid=$uid";
			  $result = mysql_query( $query, $con );
			  list( $name, $subject, $article, $userfile, $userfile2 ) = mysql_fetch_array( $result );
			  mysql_close( $con );
			?>
			<div class="form-container">
				   <form action="<?=dest_url( "./edit_db.php", $page, $uid )?>" method="post" style="padding-left:20px; padding-right:20px; margin-top:40px; margin-bottom:40px" enctype="multipart/form-data">
							<input class="form-control" minlength="1" maxlength="15" required="" aria-required="true" style="margin-bottom:25px; width:100%; height:40px; border: 1px solid #d9d9de" name="name" type="text" <?PHP
								if (session_is_registered('littlesamakfox_logon')){
								?> value="<?PHP print $littlesamakfox_logon[0];
								}?>" placeholder="아이디를 입력해주세요(최대15자)">
							<input class="form-control" minlength="1" maxlength="20" required="" aria-required="true" style="margin-bottom:25px; width:100%; height:40px; border: 1px solid #d9d9de" name="subject" type="text" value="<?=$subject?>" placeholder="제목을 입력해주세요(최대20자)">
							<input class="form-control" minlength="1" maxlength="10" required="true" aria-required="true" style="margin-bottom:25px; width:100%; height:40px; border: 1px solid #d9d9de" name="passwd" type="password" <?PHP
								if (session_is_registered('littlesamakfox_logon')){
								?> value="<?PHP print $littlesamakfox_logon[1];
								}?>" placeholder="비밀번호를 입력해주세요(최대 10자)">
							<p style="">이미지 업로드 (100MB 미만)</p>
								<p>
								<input type="hidden" name="MAX_FILE_SIZE" value="100000000">
								<input name="userfile" type="file" accept="image/*">
								<input name="userfile2" type="file" accept="image/*">
								 </p>
								<img class="img-fluid img-thumbnail" style="max-width:30%" src="/assets/img/thread/<?=$userfile?>">
								<img class="img-fluid img-thumbnail" style="max-width:30%" src="/assets/img/thread/<?=$userfile2?>">
							<textarea class="form-control" minlength="1" maxlength="1000" required="" aria-required="true" style="ime-mode:active; margin-bottom:25px; width:100%; height:40px; border: 1px solid #d9d9de; min-height:250px" name="article" placeholder="본문을 입력해주세요 (최대 1,000자)"><?=$article?></textarea>
							<div align="center">
								<input class="btn btn-primary btn-pill mr-2 mb-5" type="submit" value="수정">
							</div>
					</form>
			</div>
		</div>
