	...
	<body class="littlesamakfox">
		...
		<div class="register">
			<div class="intro">
                ...
                <h1 class="mb-4 mt-4 text-center">메타 제작</h3>
                <p class="text-center">
			...
			<?PHP
			  ...
			  #------------- 해당 글의 정보를 가져온다 -------------------#
			  $query = "select meta_name, meta_score, muserfile, meta_article, meta_initial
				from meta where meta_uid=$meta_uid";
			  $result = mysql_query( $query, $con );
			  list( $meta_name, $meta_score, $muserfile, $meta_article, $meta_initial ) = mysql_fetch_array( $result );
			  mysql_close( $con );
			?>
            <div class="form-container">
                <form action="./editmeta_db.php?meta_uid=<?=$meta_uid?>" method="post" style="padding-left:20px; padding-right:20px; margin-top:40px; margin-bottom:40px" enctype="multipart/form-data">
                        <div class="form-group">
							<input class="form-control" minlength="1" maxlength="10" required="" aria-required="true" style="margin-bottom:25px; width:100%; height:40px; border: 1px solid #d9d9de" name="meta_initial" type="text" value="<?=$meta_initial?>" placeholder="게임 이니셜을 입력해주세요">
							<input class="form-control" minlength="1" maxlength="50" required="" aria-required="true" style="margin-bottom:25px; width:100%; height:40px; border: 1px solid #d9d9de" name="meta_name" type="text" value="<?=$meta_name?>" placeholder="게임 제목을 입력해주세요">
							<input class="form-control" minlength="1" maxlength="20" required="" aria-required="true" style="margin-bottom:25px; width:100%; height:40px; border: 1px solid #d9d9de" name="meta_score" type="text" value="<?=$meta_score?>" placeholder="점수를 입력해주세요">
							<p style="">이미지 업로드 (100MB 미만)</p>
							 <p>
								<input type="hidden" name="MAX_FILE_SIZE" value="100000000">
								<input name="muserfile" type="file" accept="image/*">
							 </p>
							 <img class="img-fluid img-thumbnail" style="max-width:30%" src="/assets/img/meta/<?=$muserfile?>">
							<textarea class="form-control" minlength="1" maxlength="1000" required="" aria-required="true" style="ime-mode:active; margin-bottom:25px; width:100%; height:40px; border: 1px solid #d9d9de; min-height:250px" name="meta_article" placeholder="본문을 입력해주세요 (최대 1,000자)"><?=$meta_article?></textarea>
							<div align="center">
								<input class="btn btn-primary btn-pill mr-2 mb-5" type="submit" value="메타 수정하기">
							</div>
						</div>
				</form>
            </div>
